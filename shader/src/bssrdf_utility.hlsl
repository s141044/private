...
bool ah_sss(ray r, inout uint4 payload, hit_info info)
{
	...
	if(u < pmf)
	{
		...
		payload.y = info.primitive_index;
...
bool find_hit(ray ray, float u, out sss_payload payload, uint2 dtid = 0)
{
	...
	trace_ray(ray, info, RAY_FLAG_FORCE_NON_OPAQUE | RAY_FLAG_SKIP_CLOSEST_HIT_SHADER, INSTANCE_MASK_SUBSURFACE, ch_sss, ah_sss, ms_sss);
	if((info.w >> 24) == 0)
		return false;
