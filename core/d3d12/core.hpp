...
#ifndef NN_RENDER_D3D12_CORE_HPP
#define NN_RENDER_D3D12_CORE_HPP
...
public:
	...
	D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS build_inputs(const uint numinstances) const;
...
class render_device : public render::RenderDevice<render_device>, public render_device_impl
{
public:
	...
	void copy_texture(texture &dst, texture &src)
	{
		...
		
		{
			...
			const uint num_subresources = 1;
			mp_impl->GetCopyableFootprints(&dst_desc, 0, num_subresources, 0, layouts, nullptr, nullptr, &total_bytes);

			//ホンマはサブリソースごとにやる

			//dst_loc.Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT;
			//dst_loc.PlacedFootprint = layouts[0];
	...
	//実行
	void present(render::swapchain &swapchain_base);
	...
	//スワップチェインを作成
	swapchain_ptr create_swapchain(const HWND hwnd, const uint2 size);

	//テクスチャを作成
	texture_ptr create_texture1d(const texture_format format, const uint width, const uint mip_levels, const resource_flags flags, const void *data);
	texture_ptr create_texture2d(const texture_format format, const uint width, const uint height, const uint mip_levels, const resource_flags flags, const void *data);
	texture_ptr create_texture3d(const texture_format format, const uint width, const uint height, const uint depth, const uint mip_levels, const resource_flags flags, const void *data);
	texture_ptr create_texture_cube(const texture_format format, const uint width, const uint height, const uint mip_levels, const resource_flags flags, const void *data);
	texture_ptr create_texture1d_array(const texture_format format, const uint width, const uint depth, const uint mip_levels, const resource_flags flags, const void *data);
	texture_ptr create_texture2d_array(const texture_format format, const uint width, const uint height, const uint depth, const uint mip_levels, const resource_flags flags, const void *data);
	texture_ptr create_texture_cube_array(const texture_format format, const uint width, const uint height, const uint depth, const uint mip_levels, const resource_flags flags, const void *data);

	//バッファを作成
	buffer_ptr create_structured_buffer(const uint stride, const uint num_elements, const resource_flags flags, const void *data);
	buffer_ptr create_byteaddress_buffer(const uint size, const resource_flags flags, const void *data);
	constant_buffer_ptr create_temporary_cbuffer(const uint size, const void *data);
	constant_buffer_ptr create_constant_buffer(const uint size, const void *data);
	...
	bottom_level_acceleration_structure_ptr create_bottom_level_acceleration_structure(const render::geometry_state &gs, const raytracing_geometry_desc *descs, const uint num_descs, const bool allow_update);

	//Viewを作成
	render_target_view_ptr create_render_target_view(render::texture &tex, const texture_rtv_desc &desc);
	depth_stencil_view_ptr create_depth_stencil_view(render::texture &tex, const texture_dsv_desc &desc);
	shader_resource_view_ptr create_shader_resource_view(render::buffer &buf, const buffer_srv_desc &desc);
	shader_resource_view_ptr create_shader_resource_view(render::texture &tex, const texture_srv_desc &desc);
	unordered_access_view_ptr create_unordered_access_view(render::buffer &buf, const buffer_uav_desc &desc);
	unordered_access_view_ptr create_unordered_access_view(render::texture &tex, const texture_uav_desc &desc);
	...
	//ターゲットステートを作成
	target_state_ptr create_target_state(const uint num_rtvs, render::render_target_view* rtv_ptrs[], render::depth_stencil_view* p_dsv);

	//ジオメトリステートを作成
	geometry_state_ptr create_geometry_state(const uint num_vbs, render::buffer *vb_ptrs[8], uint offsets[8], uint strides[8], render::buffer *p_ib);

	//バインドレスリソースとして登録
	void register_bindless(render::sampler &sampler);
	void register_bindless(render::constant_buffer &cbv);
	void register_bindless(render::shader_resource_view &srv);

	//バインドレスリソースの登録解除
	void unregister_bindless(render::sampler &sampler);
	void unregister_bindless(render::constant_buffer &cbv);
	void unregister_bindless(render::shader_resource_view &srv);

	//名前を設定
	void set_name(render::buffer &buf, const wchar_t *name);
	void set_name(render::upload_buffer &buf, const wchar_t *name);
	void set_name(render::constant_buffer &buf, const wchar_t *name);
	void set_name(render::readback_buffer &buf, const wchar_t *name);
	void set_name(render::texture &tex, const wchar_t *name);
	void set_name(render::top_level_acceleration_structure &tlas, const wchar_t *name);
	void set_name(render::bottom_level_acceleration_structure &blas, const wchar_t *name);
	...
	void *validate(const render::pipeline_state &ps, const unordered_map<string, bind_resource> &resources);
	void *validate(const render::pipeline_state &ps, const render::target_state &ts, const render::geometry_state &gs, const unordered_map<string, bind_resource> &resources);
