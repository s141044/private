...
enum raytracing_geometry_flags : uint
{
...
{
	viewport() = default;
	viewport(const uint2& left_top, const uint2& size);
	viewport(const uint left, const uint top, const uint width, const uint height);
